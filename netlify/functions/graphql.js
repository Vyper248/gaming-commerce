...
const typeDefs = gql`
  ...
  type Query {
    ...
    items: [Item]
    item(id: Int): Item
    cartItems(ids: [Int]): [Item]
  }

  #Just for testing
  #type Mutation {
  #  addCategory(id: Int!, title: String!): Category
  #  addItem(id: Int!, name: String, imageURL: String, price: Float, tags: [String], description: String, releaseDate: String, category: String): Item
  #}
`;
...
const categories = [
    {
        ...
        items: [
            ...
            },
            {
                ...
                imageURL: "https://image.api.playstation.com/vulcan/ap/rnd/202109/2821/a0DyIs2SEHrYpciM1ideU1wv.jpg",
                price: 69.99,
    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                imageURL: "https://media.direct.playstation.com/is/image/sierialto/ps-move-controller-accessory-package",
                price: 59.99,
        ...
        items: [
            {
                id: 17,
                name: "Hat",
                imageURL: "https://m.media-amazon.com/images/I/813AMTmjAAL._AC_UX385_.jpg",
                price: 8.99,
                tags: ['Merchandise'],
                description: "",
                releaseDate: 'Out Now'
...
const resolvers = {
    Query: {
        ...
        category: (_, {title}) => categories.find(obj => obj.title === title),
        items: () => {
        ...
        item: (_, {id}) => {
            ...
            for (let i = 0; i < categories.length; i++) {
                ...
                for (let j = 0; j < category.items.length; j++) {
                    let itemObj = category.items[j];
                    if (itemObj.id === id) item = itemObj;
                }
            }
            return item;
        },
        cartItems: (_, {ids}) => {
            ...
            for (let i = 0; i < categories.length; i++) {
                ...
                for (let j = 0; j < category.items.length; j++) {
                    let itemObj = category.items[j];
                    if (ids.includes(itemObj.id)) items.push(itemObj);
                }
            }
            return items;
        }
    ...
    //Just testing
    // Mutation: {
    //     addCategory: (_, {id, title}) => {
    //         let newCategory = {id, title, items: []};
    //         categories.push(newCategory);
    //         return newCategory;
    //     },
    //     addItem: (_, values) => {
    //         let categoryObj = categories.find(obj => obj.title === values.category);
    //         let newItem = {...values};
